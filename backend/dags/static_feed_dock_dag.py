...
def _upload_gbfs_data():
    ...
    blob_path = f"raw/static/year={y}/month={m}/stations_{ts}.json"
    data = static_station_data()
...
with DAG(
    dag_id="dockflow_gbfs_static_every_1month",
    description="Upload GBFS Static data to GCS every  1 month",
    start_date=pendulum.datetime(2025, 8, 13, tz=local_tz),
    schedule="0 0 1 * *",
    catchup=False,
    max_active_runs=1,
    default_args={"owner": "cityride", "retries": 2},
    tags=["dockflow", "gbfs", "gcs","static"],
) as dag:
